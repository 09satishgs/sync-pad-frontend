  ...
  return (
    <aside className={`sidebar ${sidebarOpen ? 'open' : ''}`}>
      ...
      <div className="sidebar-content">
        ...
        <div className="sidebar-section-title">Current Workspace</div>
        {activeSheet ? (
          <div className={`sidebar-item ${activeTabId === 'live' ? 'active' : ''}`} onClick={() => onOpenSavedSheet({ id: 'live' })}>
        ...
        <div style={{ display: 'flex', gap: '4px', padding: '4px 8px' }}>
          <button className="btn btn-primary" style={{ flex: 1, fontSize: '11px', height: '28px' }} onClick={onSaveLiveSheet}>
            Save / Archive
          </button>
          <button className="btn btn-danger" style={{ fontSize: '11px', height: '28px' }} onClick={onDeleteLiveSheet} title="Delete and Reset Live Sheet">
            Reset
          </button>
        </div>

        {/* Saved Folders / Categories */}
        <div className="sidebar-section-title">Saved Sheets</div>

        {/* Create Category Trigger */}
        {showCatInput ? (
          <form onSubmit={handleCreateCategory} style={{ padding: '0 8px 8px 8px' }}>
            <input
              type="text"
            ...
            <div style={{ display: 'flex', gap: '4px' }}>
              <button type="submit" className="btn btn-primary" style={{ flex: 1, height: '24px', fontSize: '10px' }}>Add</button>
              <button type="button" className="btn" style={{ height: '24px', fontSize: '10px' }} onClick={() => setShowCatInput(false)}>Cancel</button>
            </div>
          </form>
        ) : (
          <button 
            className="btn" 
            style={{ width: 'calc(100% - 16px)', margin: '0 8px 8px 8px', height: '26px', fontSize: '11px', borderStyle: 'dashed' }}
              ...
              <div key={cat.id} className="category-folder">
                ...
                {isExpanded && (
                  <div className="category-sheets">
                    {catSheets.length === 0 ? (
                      <div style={{ padding: '6px 8px', color: 'var(--text-muted)', fontSize: '11px' }}>Empty</div>
                    ) : (
                      catSheets.map(sheet => (
                        <div 
                          key={sheet.id} 
        ...
        <div className="sidebar-section-title">Archived Sheets (Read-Only)</div>
        <div style={{ padding: '0 8px' }}>
      ...
      <div className="sidebar-footer">
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '140px' }}>
            <span style={{ color: 'var(--text-muted)', fontSize: '11px' }}>Active User</span>
            <div style={{ fontWeight: 600 }}>{user?.username}</div>
          </div>
          <button className="btn" style={{ fontSize: '11px', padding: '0 8px', height: '26px' }} onClick={logout}>
            Logout
          </button>
        </div>
      </div>
